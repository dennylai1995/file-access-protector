...
def read_func(file_path: str, statistic: Statistic, run_time_s: float):
    ...
    while time.time() <= stop_time:
        ...
        try:
            result = yaml_safe_load(file_path)
...
def test_concurrent_yaml_file_access():
    ...
    backup_file_path = f'{_temp_test_folder}/{os.path.basename(_test_file_path).replace(".yaml", "_backup.yaml")}'
    ...
    with open(backup_file_path, 'r') as f:
        backup_content = yaml.load(f, Loader=yaml.CLoader)
...
def test_dump_file_with_wrong_data():
    ...
    with pytest.raises(AttributeError):
        yaml_safe_dump(file_path, "")


def test_write_new_file():
    file_path = f'{_temp_test_folder}/new.yaml'
    backup_file_path = f'{_temp_test_folder}/new_backup.yaml'
...
def test_file_and_backup_corruption_in_load(mock_yaml_load):
    file_path = f'{_temp_test_folder}/{os.path.basename(_test_file_path)}'
...
@patch('yaml.load', return_value="")
def test_file_corruption_in_dump(mock_yaml_load):
    file_path = f'{_temp_test_folder}/{os.path.basename(_test_file_path)}'
...
@patch('yaml.load', return_value="")
def test_file_corruption_and_backup_not_exist_in_load(mock_yaml_load):
    file_path = f'{_temp_test_folder}/{os.path.basename(_test_file_path)}'
    backup_file_path = f'{_temp_test_folder}/{os.path.basename(_test_file_path).replace(".yaml", "_backup.yaml")}'
